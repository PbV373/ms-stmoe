...
from ms_stmoe_chla.distributed import (
    AllGather,
    split_by_rank,
    gather_sizes,
    pad_dim_to,
...
class TopNGating(Module):
    ...
    def forward(
        ...
        *_, b, group_size, dim, dtype, top_n, num_gates, eps = *x.shape, x.dtype, self.top_n, self.num_gates, self.eps
        ...
        router_input = default(router_input, x)
        assert router_input.shape == x.shape, 'router_input must have the same shape as x'
        gate_logits = self.to_gates(router_input)
...
class SparseMoEBlock(Module):
    ...
    def __init__(
        ...
        *,
        add_ff_before = False,
        add_ff_after = True,
        moe_residual_alpha_init = 1.
    ):
        super().__init__()
        if moe_residual_alpha_init < 0.:
            raise ValueError('moe_residual_alpha_init must be non-negative')

        dim = moe.dim

        self.moe = moe
        self.moe_prenorm = RMSNorm(dim)
        self.router_prenorm = RMSNorm(dim)
        self.moe_residual_alpha = nn.Parameter(torch.tensor(float(moe_residual_alpha_init)))
    ...
    def forward(
        ...
        router_input = self.router_prenorm(router_input) if exists(router_input) else None
        ...
        x = residual + self.moe_residual_alpha.to(dtype = moe_out.dtype) * float(moe_residual_scale) * moe_out
